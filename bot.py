...
def admin_menu_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📋 Mahsulotlar ro'yxati"), KeyboardButton(text="📊 Buyurtmalar")],
...
# ---------------- Mahsulot qo'shish (nom -> narx -> miqdor -> kategoriya -> tavsif -> rasm) ----------------
...
async def add_product_price(message: Message, state: FSMContext):
    ...
    await message.answer("Ombordagi miqdorini kiriting (necha dona/kg bor, masalan: 20):")
...
async def _finish_add_product(message: Message, state: FSMContext, photo_url: str):
    ...
    await state.clear()
    await message.answer(
        f"✅ Mahsulot qo'shildi: {data['name']} — {data['price']} so'm — {data['quantity']} dona"
...
async def stock_menu(message: Message, state: FSMContext):
    ...
    text = (
        ...
        "\n\nMiqdorni o'zgartirish uchun: ID MIQDOR (masalan: 3 15)\n"
        "Mahsulotni o'chirish uchun: ochir ID (masalan: ochir 3)\n"
...
async def stock_manage_input(message: Message, state: FSMContext):
    ...
    # Mahsulotni o'chirish: "ochir 3"
    if len(parts) == 2 and parts[0].lower() in ("ochir", "o'chir", "oʻchir"):
        if not parts[1].isdigit():
            await message.answer("Noto'g'ri format. Masalan: ochir 3")
            return
        pid = int(parts[1])
    ...
    await message.answer(
        ...
        "O'chirish: ochir ID (masalan: ochir 3)"
    )
