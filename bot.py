...
def admin_menu_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="✏️ Narxni tahrirlash"), KeyboardButton(text="🗑 Mahsulotni boshqarish")],
...
def manage_product_kb(product_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⛔️ Tugagan deb belgilash", callback_data=f"stockoff:{product_id}")],
        [InlineKeyboardButton(text="✅ Sotuvda deb belgilash", callback_data=f"stockon:{product_id}")],
        [InlineKeyboardButton(text="🗑 Butunlay o'chirish", callback_data=f"delprod:{product_id}")],
    ])
...
async def add_product_description(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Mahsulot rasmini shu yerga to'g'ridan-to'g'ri yuboring (telefondan tanlab),\n"
        "yoki tayyor rasm havolasini (link) yozing,\n"
        "yoki rasm kerak bo'lmasa - belgisini yuboring."
    )
...
async def add_product_photo(message: Message, state: FSMContext):
    if message.photo:
        ...
        photo_url = f"tg:{message.photo[-1].file_id}"
        ...
        await message.answer(
            "Tushunmadim. Iltimos, rasmni to'g'ridan-to'g'ri yuboring, "
            "yoki havola (link) yozing, yoki rasm kerak bo'lmasa - belgisini yuboring."
        )
        return

    data = await state.update_data(photo_url=photo_url)
...
@router.message(F.text == "🗑 Mahsulotni boshqarish")
async def manage_product_start(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    products = db.get_products(in_stock_only=False)
    if not products:
        ...
        return
    lines = [
        f"#{p['id']} {p['name']} — {p['price']} so'm "
        f"({'✅ sotuvda' if p['in_stock'] else '⛔️ tugagan'})"
        for p in products
    ]
    await state.set_state(ManageProduct.product_id)
    await message.answer(
        "Qaysi mahsulot bilan ishlamoqchisiz? Raqamini (#) yuboring:\n\n" + "\n".join(lines)
    )


@router.message(ManageProduct.product_id)
async def manage_product_select(message: Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("Iltimos, faqat mahsulot raqamini (masalan: 3) yuboring.")
        return

    product = db.get_product(int(message.text))
    if not product:
        await message.answer("Bunday raqamli mahsulot topilmadi. Qaytadan urinib ko'ring.")
        return

    await state.clear()
    status = "✅ sotuvda" if product["in_stock"] else "⛔️ tugagan"
    await message.answer(
        f"\"{product['name']}\" — {product['price']} so'm ({status})\n\nNima qilamiz?",
        reply_markup=manage_product_kb(product["id"]),
    )


@router.callback_query(F.data.startswith("stockoff:"))
async def product_mark_out(callback: CallbackQuery):
    product_id = int(callback.data.split(":")[1])
    db.set_product_stock(product_id, False)
    product = db.get_product(product_id)
    await callback.message.edit_text(f"⛔️ \"{product['name']}\" endi \"tugagan\" deb belgilandi.")
    await callback.answer("Tugagan deb belgilandi")


@router.callback_query(F.data.startswith("stockon:"))
async def product_mark_in(callback: CallbackQuery):
    product_id = int(callback.data.split(":")[1])
    db.set_product_stock(product_id, True)
    product = db.get_product(product_id)
    await callback.message.edit_text(f"✅ \"{product['name']}\" endi \"sotuvda\" deb belgilandi.")
    await callback.answer("Sotuvda deb belgilandi")
...
async def product_delete(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"🗑 \"{name}\" katalogdan butunlay o'chirildi.")
    await callback.answer("O'chirildi")


@router.message(F.text == "✏️ Narxni tahrirlash")
async def edit_price_start(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    products = db.get_products(in_stock_only=False)
    if not products:
        await message.answer("Katalog bo'sh, tahrirlash uchun mahsulot yo'q.")
        return
    lines = [f"#{p['id']} {p['name']} — {p['price']} so'm" for p in products]
    await state.set_state(EditPrice.product_id)
    await message.answer(
        "Narxini o'zgartirmoqchi bo'lgan mahsulot raqamini (#) yuboring:\n\n" + "\n".join(lines)
    )


@router.message(EditPrice.product_id)
async def edit_price_get_id(message: Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("Iltimos, faqat mahsulot raqamini (masalan: 3) yuboring.")
        return
    ...
    await message.answer(
        f"\"{product['name']}\" uchun hozirgi narx: {product['price']} so'm.\n"
        f"Yangi narxni kiriting (faqat son):"
    )
...
async def edit_price_save(message: Message, state: FSMContext):
    ...
    data = await state.update_data(new_price=int(message.text))
    db.update_product_price(data["product_id"], data["new_price"])
    product = db.get_product(data["product_id"])

    await state.clear()
    await message.answer(
        f"✅ Narx yangilandi: \"{product['name']}\" — endi {product['price']} so'm",
        reply_markup=admin_menu_kb(),
    )
...
async def photo_proxy(request: web.Request):
    """Admin Telegram orqali yuborgan rasmlarni saytga chiqarib beradi."""
    ...
    try:
        ...
        logger.warning(f"Rasmni yuklab bo'lmadi ({file_id}): {e}")
...
def create_app() -> web.Application:
    ...
    app.router.add_get("/photo/{file_id}", photo_proxy)
