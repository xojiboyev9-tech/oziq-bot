...
def init_db():
    ...
    """)
    # Eski bazalarda "quantity" ustuni bo'lmasligi mumkin - shuning uchun
    # xavfsiz migratsiya qilamiz (ustun allaqachon bo'lsa xatoni e'tiborsiz qoldiramiz).
    try:
...
def seed_sample_products():
    ...
    if cur.fetchone()["c"] == 0:
        samples = [
            ("Non", 4000, "Issiq, yangi pishirilgan non", "Non mahsulotlari", "", 30),
            ("Sut 1L", 12000, "Tabiiy sigir suti", "Sut mahsulotlari", "", 20),
            ("Tuxum (10 dona)", 18000, "Fermer tuxumi", "Sut mahsulotlari", "", 15),
            ("Olma 1kg", 9000, "Qizil olma", "Meva-sabzavot", "", 25),
        ]
        cur.executemany(
            "INSERT INTO products (name, price, description, category, photo_url, quantity, in_stock) "
            "VALUES (?,?,?,?,?,?,1)",
            samples,
        )
        conn.commit()
...
def add_product(name, price, quantity=0, description="", category="Boshqa", photo_url=""):
    conn = get_conn()
    cur = conn.cursor()
    ...
    cur.execute(
        "INSERT INTO products (name, price, description, category, photo_url, quantity, in_stock) "
        "VALUES (?,?,?,?,?,?,?)",
        (name, price, description, category, photo_url, quantity, in_stock),
    )
    conn.commit()
    pid = cur.lastrowid
    conn.close()
...
def get_products(in_stock_only=True):
    ...
    cur = conn.cursor()
    if in_stock_only:
        cur.execute("SELECT * FROM products WHERE in_stock=1 AND quantity>0 ORDER BY category, id")
    else:
...
def set_product_photo(product_id, photo_url):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE products SET photo_url=? WHERE id=?", (photo_url, product_id))
    conn.commit()
    conn.close()


def set_product_quantity(product_id, quantity: int):
    """Ombordagi miqdorni yangilaydi. 0 yoki manfiy bo'lsa mahsulot avtomatik 'tugagan' bo'ladi."""
    conn = get_conn()
    cur = conn.cursor()
    in_stock = 1 if quantity > 0 else 0
    cur.execute("UPDATE products SET quantity=?, in_stock=? WHERE id=?", (quantity, in_stock, product_id))
    conn.commit()
    conn.close()
